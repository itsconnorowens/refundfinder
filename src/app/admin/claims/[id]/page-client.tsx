...
export default function ClaimDetailClientPage({ id }: { id: string }) {
  ...
  const generateSubmission = async () => {
    try {
      ...
      const response = await fetch(`/api/admin/claims/${id}/generate-submission`, {
        method: 'POST',
      });
      const data = await response.json();
  ...
  const updateStatus = async () => {
    try {
      ...
      const response = await fetch(`/api/admin/claims/${id}/status`, {
        method: 'PUT',
  ...
  const markAsFiled = async () => {
    ...
    try {
      setUpdatingStatus(true);
      const response = await fetch(`/api/admin/claims/${id}/file`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ 
          airlineReference, 
