...
`;
  ...
  return {
    type: 'cover_letter',
    filename: `cover-letter-${airlineConfig.airlineCode.toLowerCase()}-${claim.claimId.toLowerCase()}.txt`,
...
Generated by: RefundFinder System
`;

  return {
    type: 'document_checklist',
    filename: `document-checklist-${airlineConfig.airlineCode.toLowerCase()}-${claim.claimId.toLowerCase()}.txt`,
    content: content.trim(),
    mimeType: 'text/plain',
    size: content.length,
  };
...
export function generateAllDocuments(
  ...
  // Generate cover letter for postal submissions
  if (airlineConfig.submissionMethod === 'postal') {
...
export function generateAdminNotificationEmail(
  ...
  switch (notificationType) {
    ...
    case 'overdue':
      ...
      content = `
        ...
        <p><strong>Action Required:</strong> Please file this claim immediately to meet our 48-hour promise.</p>
  ...
  return {
    ...
    filename: `admin-notification-${claim.claimId.toLowerCase()}-${notificationType}.html`,
    content: `
      <!DOCTYPE html>
      <html>
      <head>
